...
const actRegisteredReducer = (
  state: number[] = initState,
  action: { type: string; data: [] },
) => {
  switch (action.type) {
    case 'ADD_REGISTERED_ACT':
      return [...state, action.data];
    case 'CLEAN_REGISTERED_ACT':
      return state; // 清除所有的已报名活动状态，退出登录时
    case 'INIT_REGISTERED_ACT':
      return [...action.data];
    default:
      return state;
  }
  ...
  return async dispatch => {
    try {
      ...
      acts.forEach(async act => {
        if ((await actIsSignUpApi(act.id, userID)).data) {
          //每个活动查询是否报名
